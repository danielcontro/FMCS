...
# Global dictionary for memorizing computed reaches
...
def reach_from_region(
    ...
    `region` using the `method` given and filtering them using `filter`.
...
def method_fixpoint(ts: BddFsm, method: Method, region: BDD) -> BDD:
    """
    Computes a fixpoint of the `method` given starting from `region`.

    Returns the fixpoint computed.
    """
    if method == Method.POST:
        fixpoint_method = ts.post
    elif method == Method.PRE:
        fixpoint_method = ts.pre
    current = region
...
def GF_f(
    ...
    """
    Computes the states of the `ts` transition system within the `filter`
    region where the base formula `f` is repeatable starting from `region`.

    Returns the region of states where the base formula `f` is repeatable
    and the region of states from which the repeatable region can be reached.
    """
...
def meet_GF_f(
    ...
    for fi in iter(f_set):
        ...
        if not result:
            return None
        # There exist a set of states satisfying (fi & GF fi)
...
def get_index_of(region: BDD, frontiers: Frontiers) -> Optional[int]:
    ...
    `region` or None otherwise.
    """
    for idx, frontier in enumerate(frontiers):
        if frontier.intersected(region):
            return idx
    return None
...
def get_path(
    ...
    if not first_reach.intersected(end):
        return None

    if (end_index := get_index_of(end, frontiers)) == None:
        return None
...
def recursive_get_loop_through_regions(
    ...
    if start_region.intersected(regions[0]):
        start_region = start_region & regions[0]
    # restricts the starting region to the intersection between itself and the initial states if it isn't empty
    if start_region.intersected(ts.init):
        start_region = start_region & ts.init
    loop = None
    while loop == None:
        ...
        loop = aux_recursive_get_loop_through_regions(
            ts, start, start, regions[:-1], filter
        )
    return loop


def aux_recursive_get_loop_through_regions(
    ts: BddFsm, start: State, prev: State, regions: List[BDD], filter: Filter
) -> Optional[Path]:
    match regions:
        case []:
            return get_path(ts, prev, start, filter, True)
        case [f_rec, *regions]:
            segment = None
            loop_segment = None
            current_region = f_rec & filter
            while loop_segment == None:
                _, frontiers = reach_from_region(ts, prev, Method.POST, filter)
                ...
                if index == None:
                    return None
                next_state = ts.pick_one_state(
                    current_region & frontiers[index])
                segment = get_path(ts, prev, next_state, filter)
                if segment != None:
                    loop_segment = aux_recursive_get_loop_through_regions(
                        ts, start, next_state, regions, filter
                    )
...
def get_loop_through_regions(
    ...
    while start_region.isnot_false():
        ...
        if not reach_from_region(ts, ts.post(start), Method.POST, filter)[
            0
        ].intersected(start):
            continue
...
def get_counterexample(
    ...
    if DEBUG:
        print("Loop region size:", ts.count_states(loopable_region))
    ...
    if DEBUG:
        ...
        if not prefix[-1].equal(loop[0]) or not prefix[-1].equal(loop[-1]):
            print("Loop not aligned")
...
def check_explain_gr1_spec(
    ...
    whether `spec` is satisfied, and the second element is either `None` if
    the first element is `True``, or an execution of the SMV model violating
    `spec` otherwise.

    The execution is a tuple of alternating states and inputs, starting
    and ending with a state. The execution is looping if the last state is
    somewhere else in the sequence. States and inputs are represented by
    dictionaries where keys are state and inputs variable of the loaded SMV
    model, and values are their value.
    """
    ...
    if DEBUG:
        if reach.equal(ts.reachable_states | ts.init):
            print("Reachability: correct")
        else:
            print("Reachability: wrong")

    """
    For each `g_j` in `g_set` look for an execution satisfying
        (& (GF f)) & (FG !g_j)
    """
    for gj in iter(g_set):
        # Reachable states satisfying !gj
        not_gj_reachable = reach & (-spec_to_bdd(ts, gj))
        # Post fixpoint of reachable states satisfying !gj
        not_gj_post_fixpoint = method_fixpoint(
            ts, Method.POST, not_gj_reachable)
        """
        If `not_gj_post_fixpoint` is empty there is no state satisfying !gj
        reachable from other states satisfying !gj.
        In this case there isn't a reachable loop with G !gj as invariant
        hence there can't be a counterexample with the current gj thus
        proceed with gj+1
        """
        if not_gj_post_fixpoint.is_false():
            continue

        """
        Compute the list of regions in `not_gj_post_fixpoint` satisfying
        &( GF f)
        """
        result = meet_GF_f(ts, f_set, not_gj_post_fixpoint)
        """
        If it isn't None there's at least an execution for the current gj
        satisfying !spec hence get a counterexample,
        otherwise proceed with gj+1
        """
        if result:
            f_set_recurs, f_set_recur_loopable_region = result
            if DEBUG:
                print("G not respected:", gj)
            if not (
                result := get_counterexample(
            ...
            if DEBUG:
                ...
                if not ts.init.intersected(states[0]):
                    print("First state of counterexample not in initial states")
            return False, counterexample
    if DEBUG:
        check_verified, check_counter_example = pynusmv.mc.check_explain_ltl_spec(
            spec)
        if check_verified:
            print("GR1 satisfiability correct")
        else:
    ...
    """
    If there's no gj such that there exists an execution of `ts` satisfying
    (&(GF f)) & (FG !gj)
    then `ts` doesn't satisfy (&(GF f)) & (V (FG !g))
    hence every execution satisfies
    !(&(GF f)) V (&(GF g)) = (&(GF f)) -> (&(GF g))
    thus `ts` satisfies spec
    """
    return True, None
